...
pub fn build_router(registry: Arc<IndexRegistry>) -> Router {
    Router::new()
        ...
        .route("/_cluster/settings", get(cluster::cluster_settings).put(cluster::put_cluster_settings))
        ...
        // Aliases
        .route("/_aliases", get(cluster::get_all_aliases).post(cluster::post_aliases))
        ...
        .route("/{index}/_alias/{alias}", put(cluster::put_alias).delete(cluster::delete_alias))
