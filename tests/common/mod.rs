//! Shared test infrastructure: auto-starts an es-qlite server for integration tests.
...
/// Returns the base URL (e.g. "http://127.0.0.1:12345") of a running es-qlite server.
...
pub async fn ensure_server() -> &'static str {
    SERVER_URL.get_or_init(|| {
        ...
        let data_dir = std::env::temp_dir().join(format!("es-qlite-test-{port}"));
        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                let registry = Arc::new(es_qlite::storage::registry::IndexRegistry::new(
                    data_dir_clone,
                ));
                registry
                    .load_existing()
                ...
                let app = es_qlite::server::build_router(registry);
